...
**Why:** Live provider `/models` catalogs contain callable-looking entries that reject chat requests, while sending the same vision image twice can turn a healthy model into a 429 failure during one debate. Counting the vision result twice can also override a clear majority with an inconsistent LLM synthesis. Query-string keys leak through request errors and logs.
